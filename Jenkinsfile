    ...
    stages {
        stage('Build on k8 ') {
            steps {           
		        ...
		        sh 'ls -ltr'
                        sh 'pwd'
                        sh '/usr/local/bin/helm upgrade --install pet petclinic  --set image.repository=registry.hub.docker.com/rafaelgd/projectcicd --set image.tag=16'
              		script {	
			def dyna_json = """
			{
			  "title" : "PetClinic",
			  ...
			  "properties": {
			    ...
			    "dt.event.deployment.version": "1.4",
